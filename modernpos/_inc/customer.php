...
// Cliente padrão (balcão) da loja atual
$walkin_customer_id = (int)get_preference('walkin_customer_id');
if ($walkin_customer_id <= 0) {
...
// Check customer existance by id (filtrado por tenant/lojas)
function validate_existance($request, $id = 0)
{
  ...
  if (!empty($request->post['customer_email'])) {
    if (!empty($store_ids)) {
      // Modo SaaS: verificar apenas clientes das lojas do tenant
      $placeholders = implode(',', array_fill(0, count($store_ids), '?'));
      $sql = "SELECT DISTINCT c.customer_id FROM `customers` c 
              INNER JOIN `customer_to_store` c2s ON c.customer_id = c2s.customer_id 
              WHERE c.`customer_email` = ? AND c.`customer_id` != ? AND c2s.`store_id` IN ($placeholders)";
      $params = array_merge(array($request->post['customer_email'], $id), $store_ids);
  ...
  if (!empty($request->post['customer_mobile'])) {
    if (!empty($store_ids)) {
      ...
      $sql = "SELECT DISTINCT c.customer_id FROM `customers` c 
              INNER JOIN `customer_to_store` c2s ON c.customer_id = c2s.customer_id 
              WHERE c.`customer_mobile` = ? AND c.`customer_id` != ? AND c2s.`store_id` IN ($placeholders)";
      $params = array_merge(array($request->post['customer_mobile'], $id), $store_ids);
      $statement = db()->prepare($sql);
      $statement->execute($params);
    } else {
      // Modo legacy: validação global
...
// Quick Create customer (simplified - for POS)
if ($request->server['REQUEST_METHOD'] == 'POST' AND isset($request->post['action_type']) AND $request->post['action_type'] == 'QUICKCREATE')
{
  try {
    ...
    if (function_exists('can_create_customer') && !can_create_customer()) {
      $limitInfo = function_exists('get_limit_info') ? get_limit_info('customers') : array('limit' => 'ilimitado');
      throw new Exception('Limite de clientes atingido! Seu plano permite até ' . $limitInfo['limit'] . ' clientes. Faça upgrade do plano para continuar.');
    }
    ...
    // Validate customer mobile (required for quick create)
    if (empty($request->post['customer_mobile']) || !valdateMobilePhone($request->post['customer_mobile'])) {
      throw new Exception('Por favor, informe um telefone válido');
    }
    ...
    if (!empty($request->post['customer_mobile'])) {
      if (!empty($store_ids)) {
        ...
        $sql = "SELECT DISTINCT c.customer_id FROM `customers` c 
                INNER JOIN `customer_to_store` c2s ON c.customer_id = c2s.customer_id 
                WHERE c.`customer_mobile` = ? AND c2s.`store_id` IN ($placeholders)";
        $params = array_merge(array($request->post['customer_mobile']), $store_ids);
        $statement = db()->prepare($sql);
        $statement->execute($params);
      } else {
        $statement = db()->prepare("SELECT * FROM `customers` WHERE `customer_mobile` = ?");
        $statement->execute(array($request->post['customer_mobile']));
      }
      if ($statement->rowCount() > 0) {
        throw new Exception('Este número de telefone já está cadastrado');
      }
    }
    ...
    $post_data = array(
      ...
      'customer_email' => isset($request->post['customer_email']) && !empty($request->post['customer_email']) 
        ? $request->post['customer_email'] 
        : 'auto_' . unique_id(8) . '@cliente.local',
      'dob' => isset($request->post['dob']) ? $request->post['dob'] : '',
      'customer_sex' => isset($request->post['customer_sex']) ? $request->post['customer_sex'] : 1,
      'customer_age' => isset($request->post['customer_age']) ? $request->post['customer_age'] : '',
      'customer_address' => '',
      'customer_city' => '',
      'customer_state' => '',
      'customer_country' => isset($request->post['customer_country']) ? $request->post['customer_country'] : 'Brazil',
      'gtin' => '',
      'customer_store' => isset($request->post['customer_store']) ? $request->post['customer_store'] : array(store_id()),
      'status' => 1,
      'sort_order' => 0,
      'credit_balance' => 0
    );
    ...
    $customer_id = $customer_model->addCustomer($post_data);
...
// Get customer data by ID (for edit modal)
if ($request->server['REQUEST_METHOD'] == 'GET' AND isset($request->get['action_type']) AND $request->get['action_type'] == 'GETDATA')
{
  try {
    ...
    // Get customer stores
    $stores = array();
    $statement = db()->prepare("SELECT `store_id` FROM `customer_to_store` WHERE `customer_id` = ?");
...
// Update customer mobile number only (for quick update from POS)
if ($request->server['REQUEST_METHOD'] == 'POST' AND isset($request->post['action_type']) AND $request->post['action_type'] == 'UPDATEMOBILE')
{
  try {
    ...
    // Cliente padrão (balcão) da loja não pode ser editado
    if ($id === (int)$walkin_customer_id) {
      throw new Exception('Não é possível editar o cliente padrão do sistema.');
    }

    // Get the new mobile number
    $customer_mobile = isset($request->post['customer_mobile']) ? trim($request->post['customer_mobile']) : '';
    
    // Validate mobile if not empty
    if (!empty($customer_mobile) && !valdateMobilePhone($customer_mobile)) {
    ...
    if (!empty($customer_mobile)) {
      ...
      if (!empty($store_ids)) {
        ...
        $sql = "SELECT DISTINCT c.customer_id FROM `customers` c 
                INNER JOIN `customer_to_store` c2s ON c.customer_id = c2s.customer_id 
                WHERE c.`customer_mobile` = ? AND c.`customer_id` != ? AND c2s.`store_id` IN ($placeholders)";
        $params = array_merge(array($customer_mobile, $id), $store_ids);
        $statement = db()->prepare($sql);
        $statement->execute($params);
      } else {
        $statement = db()->prepare("SELECT * FROM `customers` WHERE `customer_mobile` = ? AND `customer_id` != ?");
        $statement->execute(array($customer_mobile, $id));
      }
      if ($statement->rowCount() > 0) {
        throw new Exception('Este número de telefone já está cadastrado para outro cliente.');
      }
    }
    ...
    header('Content-Type: application/json');
    echo json_encode(array('msg' => 'Telefone atualizado com sucesso!', 'id' => $id, 'customer_mobile' => $customer_mobile));
    exit();
...
if ($request->server['REQUEST_METHOD'] == 'POST' AND isset($request->post['action_type']) AND $request->post['action_type'] == 'CREATE')
{
  try {
    ...
    if (!can_create_customer()) {
      $limitInfo = get_limit_info('customers');
      throw new Exception('Limite de clientes atingido! Seu plano permite até ' . $limitInfo['limit'] . ' clientes. Faça upgrade do plano para continuar.');
    }
    // ========== FIM: Verificação de Limite SaaS ==========
...
if ($request->server['REQUEST_METHOD'] == 'POST' AND isset($request->post['action_type']) AND $request->post['action_type'] == 'DELETE') 
{
  try {
    ...
    // Cliente padrão (balcão) da loja não pode ser excluído
    if ((int)$id === (int)$walkin_customer_id) {
      throw new Exception(trans('error_delete_permission'));
    }
    ...
    // Cliente padrão (balcão) da loja não pode ser excluído
    if ((int)$request->post['customer_id'] === (int)$walkin_customer_id) {
...
// Fetch all customers (for select dropdowns like Gift Card)
if (isset($request->get['action_type']) AND $request->get['action_type'] == 'FETCHALL') {
  try {
    $exclude_walkin = isset($request->get['exclude']) && $request->get['exclude'] == '1';
    
    // Buscar todos os clientes ativos da loja
    $sql = "SELECT c.customer_id, c.customer_name, c.customer_mobile, c.customer_email 
            FROM customers c 
            INNER JOIN customer_to_store c2s ON c.customer_id = c2s.customer_id 
            WHERE c2s.store_id = ? AND c2s.status = 1";
    
    // Excluir cliente balcão se solicitado
    if ($exclude_walkin && $walkin_customer_id > 0) {
      $sql .= " AND c.customer_id != " . (int)$walkin_customer_id;
    }
    ...
    // Converter customer_id para inteiro para garantir compatibilidade com ng-options
...
$columns = array(
  ...
  array(
      'db'        => 'customer_id',
      'dt'        => 'btn_edit',
      'formatter' => function( $d, $row ) use ($walkin_customer_id) {
        // Bloqueia edição de clientes globais/padrão (ex: Cliente Balcão)
        if ((int)$row['customer_id'] === (int)$walkin_customer_id) {
          return '<button class="btn btn-sm btn-block btn-default" type="button" disabled title="Cliente padrão não pode ser editado"><i class="fa fa-fw fa-pencil"></i></button>';
        }
        $customer_name = isset($row['customer_name']) ? $row['customer_name'] : '';
        if (is_global_item($customer_name, 'customer')) {
          return '<button class="btn btn-sm btn-block btn-default" type="button" disabled title="Cliente global não pode ser editado"><i class="fa fa-fw fa-pencil"></i></button>';
        }
  ...
  array(
      ...
      'dt'        => 'btn_delete',
      'formatter' => function( $d, $row ) use ($walkin_customer_id) {
        // Bloqueia exclusão de clientes globais/padrão (ex: Cliente Balcão)
        if ((int)$row['customer_id'] === (int)$walkin_customer_id) {
          return '<button class="btn btn-sm btn-block btn-default" type="button" disabled title="Cliente padrão não pode ser excluído"><i class="fa fa-fw fa-trash"></i></button>';
        }
        $customer_name = isset($row['customer_name']) ? $row['customer_name'] : '';
        if (is_global_item($customer_name, 'customer')) {
          return '<button class="btn btn-sm btn-block btn-default" type="button" disabled title="Cliente global não pode ser excluído"><i class="fa fa-fw fa-trash"></i></button>';
        }
