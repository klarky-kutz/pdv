...
if($request->server['REQUEST_METHOD'] == 'POST' && isset($request->post['action_type']) && $request->post['action_type'] == 'DELETE') 
{
  try {
    ...
    // Bloqueia remoção da moeda BRL (Real Brasileiro) como padrão do sistema
    $stmtCode = db()->prepare("SELECT code FROM currency WHERE currency_id = ? LIMIT 1");
    $stmtCode->execute(array($id));
    $codeRow = $stmtCode->fetch(PDO::FETCH_ASSOC);
    $code    = isset($codeRow['code']) ? strtoupper($codeRow['code']) : '';
    if ($code === 'BRL') {
      throw new Exception(trans('error_delete_permission'));
    }
...
$columns = array(
  ...
  array( 
    ...
    'dt' => 'btn_edit' ,
    'formatter' => function($d, $row) {
      // Bloqueia edição da moeda BRL e moeda demo ID=1
  ...
  array( 
    ...
    'dt' => 'btn_delete' ,
    'formatter' => function($d, $row) {
      // Bloqueia exclusão da moeda BRL e moeda demo ID=1
      if (DEMO && $row['currency_id'] == 1) {          
        return '<button class="btn btn-sm btn-block btn-default" type="button" disabled><i class="fa fa-trash"></i></button>';
      }
      if (strtoupper($row['code']) === 'BRL' || $row['currency_id'] == currency_id()) {
