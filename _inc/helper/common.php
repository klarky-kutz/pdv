...
{
	/**
	 * Unserialize data with error handling and JSON support
	 * 
	 * @param mixed $data Data to unserialize (serialized PHP or JSON)
	 * @return array|mixed Unserialized data or empty array on failure
	 */
	function valid_unserialize($data)
	{
		// Retornar array vazio se dados estiverem vazios
		if (empty($data)) {
			return array();
		}
		...
		if (is_string($data)) {
			...
			// Detectar JSON por caracteres iniciais
			if ($trimmed !== '' && ($trimmed[0] === '{' || $trimmed[0] === '[')) {
				$jsonDecoded = json_decode($trimmed, true);
				if (json_last_error() === JSON_ERROR_NONE) {
					return is_array($jsonDecoded) ? $jsonDecoded : array();
				}
			}
		}
		
		// Tentar unserialize com supressão de erros
		$result = @unserialize($data);
		
		// Se unserialize falhou, retornar array vazio
		if ($result === false && $data !== serialize(false)) {
			// Log do erro para debug (opcional)
			if (function_exists('error_log')) {
				error_log('valid_unserialize: Failed to unserialize data. Length: ' . strlen($data));
			}
			return array();
		}
...
/**
 * Verifica se um item é global/padrão e não deve ser editado
 * Itens globais são compartilhados entre lojas e sua edição afetaria outras lojas
 * 
 * @param string $name Nome do item a verificar
 * @param string $type Tipo do item (product, category, brand, unit, taxrate, box, customer, supplier)
 * @return bool True se é item global/padrão
 */
if (!function_exists('is_global_item'))
{
	function is_global_item($name, $type = null)
	{
		if (empty($name)) {
			return false;
		}
		...
		// Verifica prefixos globais
		$global_prefixes = array('[Global]', '[Demo]', '[Padrão]', '[Default]');
		...
		// Nomes exatos de itens padrão que não devem ser editados
		$default_items = array(
			// Clientes
			'Cliente Balcão',
			'Walk-in Customer',
