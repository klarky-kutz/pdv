<?php
// =======================================================
// SaaS Feature Gating: carregar features permitidas do plano
// =======================================================
$saasAllowedKeys = array();
$saasIsPermissive = true;
...
if (class_exists('SaasLimitsBridge') && function_exists('db')) {
    try {
        ...
        if ($tenantId > 0) {
            $features = SaasLimitsBridge::getPlanFeatures(db(), $tenantId);
            $saasIsPermissive = in_array('*', $features, true);
            if (!$saasIsPermissive) {
                $saasAllowedKeys = array_fill_keys($features, true);
            }
        }
    } catch (Throwable $e) {
        // Fail-safe: se der erro, libera tudo (compat)
        $saasIsPermissive = true;
    }
}

if (!function_exists('saas_is_permission_allowed')) {
    function saas_is_permission_allowed($permKey) {
        global $saasIsPermissive, $saasAllowedKeys;
        if ($saasIsPermissive) return true;
        return isset($saasAllowedKeys[(string)$permKey]);
    }
}
...
$permissions = array(
    ...
    'concierge_groups' => array(
        'concierge_groups_access' => 'Acesso ao Módulo',
        'concierge_groups_ai_create' => 'Usar IA na Criação',
        'concierge_groups_manage' => 'Gerenciar Grupos',
    ),
);

// =======================================================
// SaaS Painel: modo catálogo (não renderiza HTML)
// =======================================================
if (!empty($GLOBALS['SAAS_PERMISSION_CATALOG_ONLY'])) {
    return $permissions;
}
?>
                    ...
                    <div class="well well-sm permission-well">
                        <div filter-list="search_<?php echo $type; ?>">
                            <?php foreach ($lists as $key => $name) : ?>
                                <?php
                                    $isAllowed = saas_is_permission_allowed($key);
                                    $disabledAttr = $isAllowed ? '' : ' disabled';
                                    $disabledClass = $isAllowed ? '' : ' perm-disabled';
                                    $disabledTitle = $isAllowed ? '' : ' title="Bloqueado pelo plano"';
                                ?>
                                <div class="<?php echo $disabledClass; ?>">
                                    <input type="checkbox" class="<?php echo $type; ?>" id="<?php echo $key; ?>"
                                           value="true"
                                           name="access[<?php echo $key; ?>]"<?php echo isset($the_permissions['access'][$key]) && $isAllowed ? ' checked' : null; ?><?php echo $disabledAttr; ?>>
                                    <label for="<?php echo $key; ?>"<?php echo $disabledTitle; ?>><?php echo ucfirst($name); ?><?php if (!$isAllowed): ?> <i class="fa fa-lock" style="color:#999;font-size:11px;"></i><?php endif; ?></label>
                                </div>
                            <?php endforeach; ?>
                        </div>
                    </div>
...
<style>
/* SaaS Feature Gating: estilo para permissões bloqueadas pelo plano */
.perm-disabled {
    opacity: 0.5;
    pointer-events: none;
}
