...
function (
    ...
    $(document).delegate("#create-customer-submit", "click", function(e) {
        ...
        if (typeof saasCustomerLimit !== 'undefined' && saasCustomerLimit.is_saas && !saasCustomerLimit.unlimited && !saasCustomerLimit.can_create) {
            if (typeof showSaasLimitModal === 'function') {
                showSaasLimitModal('customers', saasCustomerLimit.current, saasCustomerLimit.limit);
            } else {
                window.toastr.error('Limite de clientes do plano atingido!', 'Erro!');
            }
            return false;
        }
        ...
        then(function(response) {
            ...
            if (typeof refreshSaasLimit === 'function') {
                refreshSaasLimit('customers', function(newLimit) {
                    if (newLimit && !newLimit.can_create && !newLimit.unlimited) {
                        // Limite atingido - recarregar página para mostrar bloqueio
                        window.toastr.warning('Você atingiu o limite de clientes do seu plano!', 'Limite Atingido!');
                        setTimeout(function() {
                            location.reload();
                        }, 1500);
                    }
