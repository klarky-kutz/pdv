...
if ($request->server['REQUEST_METHOD'] == 'POST' && isset($request->post['action_type']) && $request->post['action_type'] == 'DELETE') 
{
  try {
    ...
    // Métodos canônicos do sistema nunca podem ser excluídos (mesmo via requisição manual)
    $statement = db()->prepare("SELECT `code_name` FROM `pmethods` WHERE `pmethod_id` = ? LIMIT 1");
    $statement->execute(array($id));
    $row = $statement->fetch(PDO::FETCH_ASSOC);
    if ($row && in_array($row['code_name'], array('cod', 'credit', 'gift_card', 'bkash', 'visa_card', 'card_credit', 'pix', 'cash', 'card_debit'))) {
      throw new Exception(trans('error_delete_permission'));
    }
...
$columns = array(
  ...
  array( 
    ...
    'dt' => 'btn_edit' ,
    'formatter' => function($d, $row) {
      // Métodos canônicos do sistema: não podem ser editados via painel
      if (in_array($row['code_name'], array('cod', 'credit', 'gift_card', 'bkash', 'visa_card', 'card_credit', 'pix', 'cash', 'card_debit'))) {
  ...
  array( 
    ...
    'dt' => 'btn_delete' ,
    'formatter' => function($d, $row) {
      // Métodos canônicos do sistema: não podem ser excluídos via painel
      if (in_array($row['code_name'], array('cod', 'credit', 'gift_card', 'bkash', 'visa_card', 'card_credit', 'pix', 'cash', 'card_debit'))) {
