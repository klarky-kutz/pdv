...
/**
 * Função modernizada de impressão de notas
 * Usa iframe oculto como método principal (mais compatível com navegadores modernos)
...
function printInvoice(eleID, settings) {
	...
	if (!DocumentContainer) {
		console.error('printInvoice: Elemento "' + eleID + '" não encontrado');
		window.toastr && window.toastr.error('Erro ao imprimir: conteúdo não encontrado', 'Erro');
		return false;
	}
	
	// CSS para impressão
	var printStyles = '<style type="text/css">' +
		'@media print {' +
			'body { margin: 0; padding: 0; background: #fff !important; -webkit-print-color-adjust: exact; print-color-adjust: exact; }' +
			'.no-print, .footer-actions, .modal-open.wrapper, .main-footer, .view-link, .dataTables_length, .dataTables_filter { display: none !important; }' +
		...
		'.no-print, .footer-actions { display: none !important; }' +
		'.close, .btn { display: none !important; }' +
	'</style>';
	...
	var html = '<!DOCTYPE html><html><head>' +
		...
		'<title>' + title + '</title>' +
		'<link rel="stylesheet" href="../assets/itsolution24/cssmin/main.css" type="text/css">' +
		cssLink +
		printStyles +
	'</head>' +
	'<body>' +
	...
	// Tentar usar iframe oculto (método mais confiável)
	try {
		return printViaIframe(html, title);
	} catch (e) {
		console.warn('printInvoice: Falha no método iframe, usando fallback window.open()', e);
		return printViaPopup(html, settings);
	}
}

/**
 * Imprimir via iframe oculto (método preferido)
 */
function printViaIframe(html, title) {
	// Remover iframe anterior se existir
	var existingFrame = document.getElementById('print-frame-temp');
	if (existingFrame) {
	...
	iframe.id = 'print-frame-temp';
	iframe.name = 'print-frame-temp';
	...
	var printAttempts = 0;
	var maxAttempts = 100; // Máximo 1 segundo de espera
	
	function tryPrint() {
		printAttempts++;
		
		if (iframeDoc.readyState === 'complete' || printAttempts >= maxAttempts) {
			try {
				iframe.contentWindow.focus();
				iframe.contentWindow.print();
				
				// Remover iframe após impressão (com delay para garantir conclusão)
				setTimeout(function() {
			...
			setTimeout(tryPrint, 10);
		}
	}
	
	// Iniciar tentativa de impressão após pequeno delay para CSS carregar
...
/**
 * Imprimir via popup (método fallback)
 */
function printViaPopup(html, settings) {
	var width = 750;
	var height = 650;
	
	if (settings && settings.screenSize === "fullScreen") {
		width = window.innerWidth || screen.width;
		height = window.innerHeight || screen.height;
	} else if (settings && settings.screenSize === "halfScreen") {
		width = Math.floor((window.innerWidth || screen.width) / 2);
		height = window.innerHeight || screen.height;
	}
	
	var windowSettings = "width=" + width + ",height=" + height + ",top=50,left=50,toolbars=no,scrollbars=yes,status=no,resizable=yes";
	
	var printWindow = window.open("", "PrintWindow", windowSettings);
	
	if (!printWindow) {
		window.toastr && window.toastr.warning('Popup bloqueado! Por favor, permita popups para este site e tente novamente.', 'Aviso');
		return false;
	}
	...
	var checkReady = setInterval(function() {
		...
		if (printWindow.document.readyState === "complete" || printAttempts > 100) {
