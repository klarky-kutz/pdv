...
function (
    ...
    $scope._percentage = function (amount, per)
    {
        if(false === $scope._isNumeric(amount) || false === $scope._isNumeric(per)) {
            ...
            window.toastr.error("O valor do desconto não é numérico!", "Atenção!");
            return 0;
        }
    ...
    $scope.showCustomerList = function (isClick) {
        ...
        then(function(response) {
            ...
            window.toastr.error(response.data.errorMsg, "Atenção!");
        });
    };
    ...
    $scope.addCustomer = function (customer) {
        if ($scope._isInt(customer)) {
            ...
            window.toastr.error("Ops! Cliente inválido", "Atenção!");
        }
    };
    $scope.getDefaultCustomerId = function () {
        var cid = 1;
        if (window.store && window.store.walkin_customer_id) {
            var tmp = parseInt(window.store.walkin_customer_id, 10);
            if (!isNaN(tmp) && tmp > 0) {
                cid = tmp;
            }
        }
    ...
    if (window.getParameterByName("customer_id")) {
        ...
        // add default customer (per store)
        $scope.addCustomer($scope.getDefaultCustomerId());
    }
    ...
    $scope.CustomerEditModal = function() {
        ...
        var walkinCustomerId = window.store && window.store.walkin_customer_id ? window.store.walkin_customer_id : 1;
        CustomerEditModal(customerData, walkinCustomerId);
    };
    ...
    $scope._calcTotalPayable = function ($childScope) {
        ...
        if ($scope.payable != 0 && ($scope.discountAmount >= $scope.payable)) {
            ...
            window.toastr.error("O desconto deve ser menor que o valor a pagar", "Atenção!");
        }
        if ($scope.payable != 0 && ($scope.shippingAmount >= $scope.payable)) {
            ...
            window.toastr.error("O frete deve ser menor que o valor a pagar", "Atenção!");
        }
                    ...
                    window._.map($scope.itemArray, function (item) {
                        if (item.id == response.data.p_id) {
                            ...
                            if ((qty > response.data.quantity_in_stock || $scope.itemQuantity >= response.data.quantity_in_stock) && response.data.p_type != 'service') {
                                ...
                                window.toastr.error("Este produto está sem estoque!", "Atenção!");
                                return false;
                            }
                            if (window.store.sound_effect == 1) {
                ...
                window._.map($scope.itemArray, function (item) {
                    if (item.id == id) {
                        if (item.quantity > 1) {
                            ...
                            window.toastr.error("A quantidade não pode ser menor que 1", "Atenção!");
                        }
                    }
                });
            }
    ...
    $scope.removeItemFromInvoice = function (index, id) {
        ...
        if ($scope.isEditMode) {
            if ($scope.itemArray.length <= 1) {
                ...
                window.toastr.error("O último item não pode ser removido!", "Atenção!");
                return false;
            }
        }
    ...
    $scope.loadHoldingOrder = function() {
        if (window.getParameterByName("holding_id")) {
            ...
            then(function(response) {
                ...
                window.toastr.warning(response.data.errorMsg, "Atenção!");
    ...
    $scope.clickOnPrintOrder = function () {
        if ($scope.itemArray.length <= 0) {
            ...
            window.toastr.error("Por favor, selecione pelo menos um produto", "Atenção!");
            return false;
        }
    ...
    $scope.payNow = function() {
        ...
        if (!$scope.customerName) {
            ...
            window.toastr.error("Por favor, selecione um cliente", "Atenção!");
            return false;
        }
        
        // Tratamento para campos vazios - preencher com zero
        if ($scope.discountInput === '' || $scope.discountInput === null || $scope.discountInput === undefined || isNaN(parseFloat($scope.discountInput))) {
            $scope.discountInput = 0;
        }
        if ($scope.taxInput === '' || $scope.taxInput === null || $scope.taxInput === undefined || isNaN(parseFloat($scope.taxInput))) {
            $scope.taxInput = 0;
        }
        if ($scope.shippingInput === '' || $scope.shippingInput === null || $scope.shippingInput === undefined || isNaN(parseFloat($scope.shippingInput))) {
            $scope.shippingInput = 0;
        }
        if ($scope.othersChargeInput === '' || $scope.othersChargeInput === null || $scope.othersChargeInput === undefined || isNaN(parseFloat($scope.othersChargeInput))) {
            $scope.othersChargeInput = 0;
        }
        
        // Recalcular o total com os valores corrigidos
        $scope._calcTotalPayable();
    ...
    $.contextMenu.types.label = function(item, opt, root) {
        ...
        .on("click", "button", function() {
            ...
            if (!itemQuantity || parseFloat(itemQuantity) <= 0) {
                ...
                window.toastr.error("A quantidade deve ser maior que 0!", "Atenção!");
                return false;
            }
    ...
    $scope.createNewProduct = function () {
        // Usa a modal moderna de criação rápida
        ProductQuickCreateModal($scope);
    };

    // Modal antiga (formulário completo) - pode ser usada via menu se necessário
    $scope.createNewProductFull = function () {
    ...
    $scope.CustomerCreateModalCallback = function(data) {
        // Atualiza diretamente sem $apply pois já está dentro do ciclo Angular
    ...
    // Create new customer (using simplified quick create modal)
    $scope.createNewCustomer = function () {
        if ($scope.invoiceId) return false;
        $scope.dueAmount = 0;
        $scope.addCustomer($scope.getDefaultCustomerId());
        CustomerQuickCreateModal($scope);
    };
