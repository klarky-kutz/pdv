...
function (
    ...
    dt.dataTable({
        ...
        "buttons": [
            ...
            {
                text:      "<i class=\"fa fa-file-excel-o\" style=\"color:#1d6f42\"></i>",
                titleAttr: "Exportar Excel (Compatível com Importação)",
                action: function ( e, dt, node, config ) {
                    window.location.href = window.baseUrl + 'admin/export_product.php?format=xls' + (supplierId ? '&sup_id=' + supplierId : '');
                }
            },
            {
                text:      "<i class=\"fa fa-file-text-o\" style=\"color:#217346\"></i>",
                titleAttr: "Exportar CSV (Compatível com Importação)",
                action: function ( e, dt, node, config ) {
                    window.location.href = window.baseUrl + 'admin/export_product.php?format=csv' + (supplierId ? '&sup_id=' + supplierId : '');
                }
            },
            {
                text:      "<i class=\"fa fa-file-pdf-o\" style=\"color:#f40f02\"></i>",
                titleAttr: "Exportar PDF (Relatório)",
                action: function ( e, dt, node, config ) {
                    window.open(window.baseUrl + 'admin/export_product.php?format=pdf' + (supplierId ? '&sup_id=' + supplierId : ''), '_blank');
                }
            }
        ]
    });
    ...
    $(document).delegate("#create-product-submit", "click", function(e) {
        ...
        if (typeof saasProductLimit !== 'undefined' && saasProductLimit.is_saas && !saasProductLimit.unlimited && !saasProductLimit.can_create) {
            if (typeof showSaasLimitModal === 'function') {
                showSaasLimitModal('products', saasProductLimit.current, saasProductLimit.limit);
            } else {
                window.toastr.error('Limite de produtos do plano atingido!', 'Erro!');
            }
            return false;
        }
        ...
        then(function(response) {
            ...
            // ========== INÍCIO: Verificar limite SaaS após cadastro ==========
            if (typeof refreshSaasLimit === 'function') {
                refreshSaasLimit('products', function(newLimit) {
                    if (newLimit && !newLimit.can_create && !newLimit.unlimited) {
                        // Limite atingido - recarregar página para mostrar bloqueio
                        window.toastr.warning('Você atingiu o limite de produtos do seu plano!', 'Limite Atingido!');
                        setTimeout(function() {
                            location.reload();
                        }, 1500);
                    }
            ...
            // ========== FIM: Verificar limite SaaS após cadastro ==========
