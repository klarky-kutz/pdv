...
function (
    ...
    $scope.openProductSelector = function() {
        ProductSelectorModal({
            multiple: true,
            onSelect: function(products) {
                // Adiciona produtos selecionados à lista de transferência
                products.forEach(function(product) {
                    // Verifica se o produto já está na lista
                    var existing = window._.find($scope.transferItemArray, function(item) {
                        return item.p_id === product.p_id;
                    });
                    
                    if (!existing) {
                        $scope.transferItemArray.push({
                            id: product.p_id,
                ...
                $scope.$apply();
                window.toastr.success(products.length + ' produto(s) adicionado(s)', 'Sucesso!');
            }
        });
    };
