...
// =======================================================
// CORREÇÃO (Bug do Loop)
// Se um ID de ativação for passado, o _init.php (linha 4) já ativou
// a loja. Agora redirecionamos para o dashboard.
// =======================================================
if (isset($request->get['active_store_id']))
{
    // Verifica se o usuário queria ir para o PDV
    if (isset($request->get['redirect_to']) && $request->get['redirect_to'] == 'pos') {
        redirect(root_url().ADMINDIRNAME.'/pos.php');
    } else {
        // Se não, envia para o dashboard (Gestão)
        redirect(root_url().ADMINDIRNAME.'/dashboard.php');
    }
    exit();
}
...
// Adiciona o Controller JS (Original, necessário para a tabela)
$document->addScript('../assets/itsolution24/angular/controllers/StoreController.js');

// =======================================================
// INÍCIO DO HTML (Novo Layout)
// =======================================================
include ("../_inc/template/header_admin.php"); // Usa o NOVO header
include ("../_inc/template/sidebar_admin.php"); // Usa a NOVA sidebar
?>
