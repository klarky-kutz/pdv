...
if ($request->server['REQUEST_METHOD'] == 'POST' AND isset($request->post['action_type']) && $request->post['action_type'] == 'UPDATE')
{
  try {
    ...
    // Bloqueia edição da "Caixa Comum" (code_name caixa)
    $stmtBox = db()->prepare("SELECT code_name FROM boxes WHERE box_id = ? LIMIT 1");
    $stmtBox->execute(array($id));
    $brow = $stmtBox->fetch(PDO::FETCH_ASSOC);
    $bcode = isset($brow['code_name']) ? strtolower($brow['code_name']) : '';
    if ($bcode === 'caixa') {
      throw new Exception(trans('error_update_permission'));
    }
...
if ($request->server['REQUEST_METHOD'] == 'POST' AND isset($request->post['action_type']) && $request->post['action_type'] == 'DELETE') {
  try {
    ...
    // Bloqueia exclusão da "Caixa Comum" (code_name caixa)
    $stmtBox = db()->prepare("SELECT code_name FROM boxes WHERE box_id = ? LIMIT 1");
    $stmtBox->execute(array($id));
    $brow = $stmtBox->fetch(PDO::FETCH_ASSOC);
    $bcode = isset($brow['code_name']) ? strtolower($brow['code_name']) : '';
...
$columns = array(
  ...
  array(
    ...
    'dt'        => 'btn_edit',
    'formatter' => function( $d, $row ) {
      // Bloqueia edição de caixas globais/padrão
      ...
      $box_name = isset($row['box_name']) ? $row['box_name'] : '';
      $code_name = isset($row['code_name']) ? $row['code_name'] : '';
      if (is_global_item($box_name, 'box') || is_global_item($code_name, 'box')) {
  ...
  array(
    ...
    'dt'        => 'btn_delete',
    'formatter' => function( $d, $row ) {
      // Bloqueia exclusão de caixas globais/padrão
      if ($row['box_id'] == 1) {
