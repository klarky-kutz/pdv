...
try {
    ...
    $stmt = db()->prepare("SELECT * FROM ai_settings WHERE store_id = :tid LIMIT 30");
    $stmt->execute([':tid' => $tenantId]);
    $configs = $stmt->fetchAll(PDO::FETCH_ASSOC);
    
    echo '<h3>Configurações do ai_settings:</h3>';
    echo '<div style="background:#f8f9fa;padding:10px;border-radius:6px;font-family:monospace;max-height:500px;overflow:auto">';
    foreach ($configs as $c) {
        $key = htmlspecialchars($c['setting_name'] ?? $c['key_name'] ?? '');
        $val = htmlspecialchars($c['setting_value'] ?? $c['key_value'] ?? '');
