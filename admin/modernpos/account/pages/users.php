  ...
  <div class="modal-premium-card" style="max-width: 600px;">
    ...
    <div class="modal-premium-body" style="max-height: 70vh; overflow-y: auto;">
      ...
      <div class="form-row-premium">
        ...
        <div class="form-group-premium">
          <label class="label-premium">
            <i class="bi bi-shop ic-modal"></i> Loja Vinculada *
          </label>
          <select id="addUserStore" class="input-premium" style="padding-right: 40px;">
            <option value="">Selecione uma loja...</option>
            <?php
            if ($tenantId > 0 && !empty($availableStores)) {
                foreach ($availableStores as $store) {
                    echo '<option value="' . (int)$store['store_id'] . '">' . htmlspecialchars($store['name'], ENT_QUOTES, 'UTF-8') . '</option>';
                }
            }
            ?>
          </select>
    ...
    <div class="modal-premium-body" style="max-height: 70vh; overflow-y: auto;">
      ...
      <div class="form-row-premium">
        ...
        <div class="form-group-premium">
          ...
          <select id="editUserStore" class="input-premium" style="padding-right: 40px;">
            <option value="">Selecione uma loja...</option>
            <?php
            if ($tenantId > 0 && !empty($availableStores)) {
                foreach ($availableStores as $store) {
...
async function openEditUserModalPremium(userId, username, email, mobile, groupId, status, isOwner) {
  ...
  try {
    ...
    document.getElementById('editUserStore').value = u.store_id || '';
...
function openAddUserModal() {
  ...
  document.getElementById('addUserStore').value = '';
...
async function saveAddUserPremium() {
  ...
  const storeId = document.getElementById('addUserStore').value;
