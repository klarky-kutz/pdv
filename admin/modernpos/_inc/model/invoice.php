...
class ModelInvoice extends Model
{
    ...
    public function createInvoice($request, $store_id = null)
    {
        ...
        // Verificar se é pagamento dividido
        $is_split_payment = isset($request->post['split_payment']) && $request->post['split_payment'] == '1';
        $split_payments = isset($request->post['split_payments']) ? $request->post['split_payments'] : array();
        
        if ($paid_amount > 0) {
            if ($is_split_payment && !empty($split_payments)) {
                // Pagamento Dividido - criar múltiplos registros
                $first_pmethod_id = null;
                foreach ($split_payments as $sp) {
                    $sp_pmethod_id = isset($sp['pmethod_id']) ? (int)$sp['pmethod_id'] : 0;
                    $sp_amount = isset($sp['amount']) ? (float)$sp['amount'] : 0;
                    
                    if ($sp_pmethod_id > 0 && $sp_amount > 0) {
                        // Calcular capital proporcional
                        $sp_capital = ($total_purchase_price / ($subtotal - $discount_amount)) * ($sp_amount - (($shipping_amount + $others_charge) * ($sp_amount / $paid_amount)));
                        ...
                        $statement->execute(array($store_id, $invoice_id, $sp_pmethod_id, $sp_capital, $sp_amount, $details, $invoice_note, $sp_amount, 0, $user_id, $created_at));
                        
                        // Guardar primeiro método para selling_info
                        if ($first_pmethod_id === null) {
                            $first_pmethod_id = $sp_pmethod_id;
                        }
                        
                        // Log para cada pagamento
                        ...
                        $statement->execute(array($customer_id, $reference_no, 'sell', $sp_pmethod_id, 'Pagamento dividido', $sp_amount, $store_id, $invoice_id, $user_id, $created_at));
                    }
                }
                
                $statement = $this->db->prepare("UPDATE `selling_info` SET `pmethod_id` = ?, `checkout_status` = ? WHERE `invoice_id` = ? AND `store_id` = ?");
                $statement->execute(array($first_pmethod_id ? $first_pmethod_id : $pmethod_id, 1, $invoice_id, $store_id));
            } else {
                // Pagamento normal - único registro
                $statement = $this->db->prepare("INSERT INTO `payments` (store_id, invoice_id, pmethod_id, capital, amount, details, note, total_paid, pos_balance, created_by, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        ...
        // Log principal (só se não for split payment, pois o split já cria os logs acima)
        if (!$is_split_payment || empty($split_payments)) {
            $reference_no = generate_sell_log_ref_no('sell');
            $statement = db()->prepare("INSERT INTO `sell_logs` (customer_id, reference_no, type, pmethod_id, description, amount, store_id, ref_invoice_id, created_by, created_at) VALUES (?,?,?,?,?,?,?,?,?,?)");
        ...
        if (INSTALLMENT && $request->post['is_installment_order']) {
            // Garantir que installment_count seja pelo menos 1 para evitar divisão por zero
            ...
            $eachInstallmentAmount = $installment_count > 0 ? ($due / $installment_count) : 0;
            $eachInstallmentInterestAmount = $installment_count > 0 ? (($installment_interest_amount - $initInterestAmount) / $installment_count) : 0;
            if ($paid_amount > 0) {
