...
if ($request->server['REQUEST_METHOD'] == 'POST' AND $request->get['action_type'] == 'HOLD')
{
  try {
    ...
    if (!isset($request->post['product-item']) || !is_array($request->post['product-item']) || empty($request->post['product-item'])) {
...
if ($request->server['REQUEST_METHOD'] == 'GET' AND $request->get['action_type'] == 'HOLDINGORDERDETAILS') {
    ...
    try {
      ...
      // Usar o customer_name da query JOIN ou buscar se não existir
      $order['customer'] = !empty($order['customer_name']) ? $order['customer_name'] : 'Consumidor';
      ...
      $items = array_map(function($item) {
        $product = get_the_product($item['item_id']);
        $unit_name = '';
        if ($product && isset($product['unit_id'])) {
          $unit_name = get_the_unit($product['unit_id'], 'unit_name');
        }
        return array_merge($item, array('unit_name' => $unit_name ? $unit_name : ''));
      }, $items);
...
if ($request->server['REQUEST_METHOD'] == 'GET' AND $request->get['action_type'] == 'HOLDINGORDERDETAILSMODAL') {
    ...
    try {
      ...
      // Buscar apenas pedidos válidos (com ref_no não nulo)
      $statement = db()->prepare("SELECT `holding_info`.*, `holding_price`.*, `customers`.`customer_id`, `customers`.`customer_name`, `customers`.`customer_mobile`, `customers`.`customer_email` FROM `holding_info` 
        LEFT JOIN `holding_price` ON `holding_info`.`ref_no` = `holding_price`.`ref_no` 
        LEFT JOIN `customers` ON `holding_info`.`customer_id` = `customers`.`customer_id` 
        WHERE `holding_info`.`store_id` = ? AND `holding_info`.`ref_no` IS NOT NULL ORDER BY `holding_info`.`info_id` DESC LIMIT 0,50");
