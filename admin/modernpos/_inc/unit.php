...
// Get units list (para ProductQuickCreateModal e outros seletores) - endpoint público para dados de referência
if ($request->server['REQUEST_METHOD'] == 'GET' && isset($request->get['get_units']))
{
  try {
    // Obter store_id da sessão ou usar padrão
    $current_store_id = function_exists('store_id') ? store_id() : 1;
    
    $statement = db()->prepare("SELECT DISTINCT u.unit_id, u.unit_name, u.code_name 
      FROM units u
      INNER JOIN unit_to_store u2s ON u.unit_id = u2s.uunit_id
      WHERE u2s.store_id = ? AND u2s.status = 1
...
if ($request->server['REQUEST_METHOD'] == 'POST' AND isset($request->post['action_type']) && $request->post['action_type'] == 'UPDATE')
{
  try {
    ...
    // Bloqueia edição da unidade canônica "Uni" (por code_name)
    $stmtUnit = db()->prepare("SELECT code_name FROM units WHERE unit_id = ? LIMIT 1");
    $stmtUnit->execute(array($id));
    $urow = $stmtUnit->fetch(PDO::FETCH_ASSOC);
    $ucode = isset($urow['code_name']) ? strtolower($urow['code_name']) : '';
    if (in_array($ucode, ['uni', 'uni.'], true)) {
      throw new Exception(trans('error_update_permission'));
    }
...
if ($request->server['REQUEST_METHOD'] == 'POST' AND isset($request->post['action_type']) && $request->post['action_type'] == 'DELETE') {
  try {
    ...
    // Bloqueia exclusão da unidade canônica "Uni" (por code_name)
    $stmtUnit = db()->prepare("SELECT code_name FROM units WHERE unit_id = ? LIMIT 1");
    $stmtUnit->execute(array($id));
    $urow = $stmtUnit->fetch(PDO::FETCH_ASSOC);
    $ucode = isset($urow['code_name']) ? strtolower($urow['code_name']) : '';
...
$columns = array(
  ...
  array(
    ...
    'dt'        => 'btn_edit',
    'formatter' => function( $d, $row ) {
      // Bloqueia edição de unidades globais/padrão
      ...
      $code_name = isset($row['code_name']) ? $row['code_name'] : '';
      if (is_global_item($unit_name, 'unit') || is_global_item($code_name, 'unit')) {
        return '<button class="btn btn-sm btn-block btn-default" type="button" disabled title="Item global não pode ser editado"><i class="fa fa-pencil"></i></button>';
      }
  ...
  array(
    ...
    'dt'        => 'btn_delete',
    'formatter' => function( $d, $row ) {
      // Bloqueia exclusão de unidades globais/padrão
      if ($row['unit_id'] == 1) {
      ...
      $code_name = isset($row['code_name']) ? $row['code_name'] : '';
      if (is_global_item($unit_name, 'unit') || is_global_item($code_name, 'unit')) {
        return '<button class="btn btn-sm btn-block btn-default" type="button" disabled title="Item global não pode ser excluído"><i class="fa fa-fw fa-trash"></i></button>';
      }
