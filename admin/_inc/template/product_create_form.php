    ...
    <div class="nav-tabs-custom">
      ...
      <div class="tab-content">
        ...
        <div class="tab-pane<?php echo $tab_active == 'general' ? ' active' : null;?>" id="general">
          ...
          <?php 
          // ========== INÍCIO: Verificar se é SaaS para ocultar campo Loja ==========
          $is_saas_tenant = (function_exists('get_tenant_id_from_session') && get_tenant_id_from_session() > 0);
          $current_store_id = function_exists('store_id') ? (int)store_id() : 0;
          // ========== FIM: Verificar se é SaaS ==========
          ?>
          
          <!-- Campo Loja - Oculto para SaaS, visível para não-SaaS -->
          <div class="form-group" <?php if ($is_saas_tenant): ?>style="display:none;"<?php endif; ?>>
              ...
              <div class="well well-sm store-well"> 
                <div filter-list="search_store">
                  <?php
                    ...
                    if (empty($store_list)) {
                      // Fallback SaaS: pega lojas do tenant (StoreModel já filtra por tenant_id na sessão)
                      $storeModel = registry()->get('loader')->model('store');
                      $store_list = $storeModel->getStores();
                    }

                    // Garante que a loja atual apareça
                    if ($current_store_id) {
                      $exists_current = false;
                      foreach ($store_list as $s) {
                        if (isset($s['store_id']) && (int)$s['store_id'] === $current_store_id) {
                          $exists_current = true;
                          break;
                        }
                      }
                      if (!$exists_current) {
                        $storeModel = isset($storeModel) ? $storeModel : registry()->get('loader')->model('store');
                        $st = $storeModel->getStore($current_store_id);
                        if ($st) {
                          $store_list[] = $st;
                        }
                      }
                    ...
                    $default_selected_store_id = $current_store_id;
                    if (!$default_selected_store_id && count($store_list) === 1 && isset($store_list[0]['store_id'])) {
                      $default_selected_store_id = (int)$store_list[0]['store_id'];
                    }
                  ?>
                  <?php foreach($store_list as $the_store) : ?>                  
                    <?php if (!isset($the_store['store_id'])) continue; ?>
                    <div class="checkbox">
                      <label>                         
                        <input type="checkbox" name="product_store[]" value="<?php echo (int)$the_store['store_id']; ?>" <?php echo ((int)$the_store['store_id'] === (int)$default_selected_store_id) ? 'checked' : null; ?>>
                        <?php echo htmlspecialchars($the_store['name']); ?>
                      </label>
                    </div>
                  <?php endforeach; ?>
                </div>
...
$(document).ready(function() {
	"use strict";
  // O p_code agora é gerado via endpoint (_inc/product.php?action_type=NEXT_PCODE)
  // no ProductCreateModal (assets/itsolution24/angularmin/modal.js)
})
</script>
