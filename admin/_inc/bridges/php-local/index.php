...
function getFileManagerPathForTenant() {
    ...
    // Fallback de compatibilidade: resolve tenant pela store ativa.
    if ($tenantId <= 0 && isset($_SESSION['store_id'])) {
        ...
        if ($storeId > 0) {
            try {
                ...
                $stmt = $pdo->prepare("SELECT tenant_id FROM stores WHERE store_id = ? LIMIT 1");
